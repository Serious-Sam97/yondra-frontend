        ...
        <ToastContext.Provider value={{ pushToast }}>
            ...
            >
                {toasts.map(t => (
                    <div
                        key={t.id}
                        onClick={() => { if (t.deepLink) router.push(t.deepLink); dismiss(t.id); }}
                    ...
                    >
                        <span
                            className="cf-led"
                            aria-hidden
                            style={{ marginTop: 4, width: 8, height: 8, flexShrink: 0, background: 'var(--cf-amber)', boxShadow: '0 0 6px var(--cf-amber)' }}
                        />
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <p className="cf-label" style={{ color: 'var(--cf-phosphor)', fontWeight: 700, marginBottom: 2 }}>New notification</p>
                            <p className="text-xs" style={{ color: 'var(--cf-text)' }}>{t.message}</p>
                        </div>
                        <button
