...
import type { RevenueMonth } from "@/interfaces/RevenueReportInterface";

// "Jul" / "Jul '26" — year suffix only on January or the first bar, so a
// 12-month axis stays readable without repeating the year on every tick.
...
/** Monthly revenue as olive bars on the pale-green LCD screen. Bars scale to
 *  the peak month; hovering a bar surfaces its exact revenue/quotes/clients.
 *  A dashed line marks the mean so a spike reads at a glance. */
export default function RevenueBars({
  ...
  onHover?: (m: RevenueMonth | null) => void;
}) {
  ...
  const mean = data.length
    ? data.reduce((s, m) => s + m.revenue, 0) / data.length
    : 0;
            ...
            return (
              ...
              >
                ...
                {h > 0 && (
                  ...
                  >
                    <title>
                      {monthLabel(m.month, true)} — {money(m.revenue)} ·{" "}
                      {m.count} quote{m.count === 1 ? "" : "s"} · {m.clients}{" "}
                      client{m.clients === 1 ? "" : "s"}
                    </title>
                  </rect>
                )}
                <text
