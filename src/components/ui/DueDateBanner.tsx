...
export function DueDateBanner({
  ...
  const doneIds = new Set(
    sections.filter((s) => s.name?.toLowerCase() === "done").map((s) => s.id),
  );

  const overdue: CardInterface[] = [];
  const dueToday: CardInterface[] = [];
  for (const c of cards) {
    if (!c.due_date || dismissed.has(c.id) || doneIds.has(c.section_id))
      continue;
  ...
  const dismissAll = () =>
    setDismissed((prev) => new Set([...prev, ...all.map((c) => c.id)]));

  return (
    <div
      className="aero-column mb-4 rounded-lg overflow-hidden"
      style={{
        borderLeft: `3px solid ${overdue.length > 0 ? "var(--cf-red)" : "var(--cf-amber)"}`,
        boxShadow: `0 0 18px ${overdue.length > 0 ? "rgba(255,90,77,0.35)" : "rgba(255,176,0,0.3)"}`,
      }}
    >
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-2.5">
        <span
          ...
          style={{
            backgroundColor:
              overdue.length > 0 ? "var(--cf-red)" : "var(--cf-amber)",
            boxShadow: `0 0 8px ${overdue.length > 0 ? "var(--cf-red)" : "var(--cf-amber)"}`,
          }}
        />
        <span
          className="cf-label text-xs uppercase tracking-widest font-bold flex-1"
          style={{
            color: overdue.length > 0 ? "var(--cf-red)" : "var(--cf-amber)",
            textShadow: `0 0 8px ${overdue.length > 0 ? "var(--cf-red)" : "var(--cf-amber)"}`,
          }}
        >
          ...
          {overdue.map((c) => (
            <div key={c.id} className="flex items-center gap-2">
              <span
                ...
                style={{
                  backgroundColor: "var(--cf-red)",
                  boxShadow: "0 0 6px var(--cf-red)",
                }}
              />
              <button
              ...
              <span
                className="cf-label text-[9px] uppercase tracking-widest flex-shrink-0"
              ...
              <button
                ...
                className="text-xs cursor-pointer flex-shrink-0 hover:opacity-100"
                style={{ color: "var(--cf-text-muted)" }}
              >
                ✕
              </button>
