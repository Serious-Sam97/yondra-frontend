...
export const BOARD_TYPES: { key: BoardType; label: string; desc: string; color: string }[] = [
...
export function BoardTypeFields({ type, currency, onTypeChange, onCurrencyChange }: {
    type: BoardType;
    currency: string;
    ...
    return (
        <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-2">
                <label className="cf-label uppercase tracking-widest font-bold" style={{ fontSize: '12px', color: 'var(--cf-text-muted)' }}>Board type</label>
                        ...
                        return (
                            ...
                            >
                                <span className="flex items-center gap-1.5">
                                    <span className="rounded-full" style={{ width: 11, height: 11, background: bt.color, boxShadow: `0 0 6px ${bt.color}` }} />
                                    <span className="cf-mono uppercase font-bold" style={{ fontSize: '12px', letterSpacing: '0.08em', color: active ? 'var(--cf-text)' : 'var(--cf-text-muted)' }}>{bt.label}</span>
                                </span>
                                <span className="cf-mono" style={{ fontSize: '10px', color: 'var(--cf-text-muted)' }}>{bt.desc}</span>
                            </button>
                        );
                    })}
                </div>
            ...
            {type === 'crm' && (
                <div className="flex flex-col gap-1.5">
                    <label className="cf-label uppercase tracking-widest font-bold" style={{ fontSize: '12px', color: 'var(--cf-text-muted)' }}>Deal currency</label>
                    <select
                        value={currency}
