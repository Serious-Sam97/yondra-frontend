...
export function BoardFormModal({ board, projectId, projectColor, ownedProjects, onSave, onDelete, onClose }: {
    ...
    return (
        <div className="aero-menu rounded-2xl p-7 w-[92vw] max-w-lg flex flex-col gap-6 relative">
            ...
            <div style={{ borderBottom: '1px solid var(--cf-edge, #4a463f)' }} className="pb-3.5 flex items-center gap-2.5">
                <span className="cf-led" style={{ backgroundColor: accentColor, boxShadow: `0 0 8px ${accentColor}`, width: 9, height: 9 }} />
                <p style={{ fontSize: '13px', color: 'var(--cf-text-muted, #a39d8c)' }} className="cf-label uppercase tracking-[0.25em] font-bold">{isNew ? 'New Board' : 'Board Settings'}</p>
            </div>
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5">
                    <label style={{ fontSize: '12px', color: 'var(--cf-text-muted, #a39d8c)' }} className="cf-label uppercase tracking-widest font-bold">Name</label>
                ...
                <div className="flex flex-col gap-1.5">
                    <label style={{ fontSize: '12px', color: 'var(--cf-text-muted, #a39d8c)' }} className="cf-label uppercase tracking-widest font-bold">Description</label>
                ...
                {isNew && (
                    <BoardTypeFields type={boardType} currency={currency} onTypeChange={setBoardType} onCurrencyChange={setCurrency} />
                )}
                {ownedProjects.length > 1 && (
                    <div className="flex flex-col gap-1.5">
                        <label style={{ fontSize: '12px', color: 'var(--cf-text-muted, #a39d8c)' }} className="cf-label uppercase tracking-widest font-bold">Project</label>
                        <div className="flex items-center gap-2">
                            <span className="cf-led flex-shrink-0" style={{ backgroundColor: accentColor, boxShadow: `0 0 8px ${accentColor}`, width: 9, height: 9 }} />
                        ...
                        {moved && <p style={{ fontSize: '11px', color: 'var(--cf-amber, #ffb000)' }} className="cf-mono uppercase tracking-widest">↳ Will move to "{targetProject?.name}"</p>}
                    </div>
                )}
                <div className="flex items-center justify-between pt-3" style={{ borderTop: '1px solid var(--cf-edge, #4a463f)' }}>
                    {onDelete ? (
                        <button type="button" onClick={onDelete}
                            className="aero-btn aero-btn--magenta uppercase tracking-widest font-bold px-5 py-2.5 text-[12px]">Delete</button>
                    ) : <div />}
                    <div className="flex gap-3">
                        <button type="button" onClick={onClose}
                            className="aero-btn aero-btn--ghost uppercase tracking-widest font-bold px-5 py-2.5 text-[12px]">Cancel</button>
