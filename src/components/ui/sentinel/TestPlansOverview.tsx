...
import { type CaseStatus, STATUS_META } from "@/interfaces/QAInterface";
...
export function TestPlansOverview({
  ...
  if (loading) {
    return (
      <p
        className="cf-mono py-10 text-center"
        style={{ fontSize: "12px", color: "var(--cf-text-dim)" }}
      >
        Loading plans…
      </p>
    );
  ...
  if (plans.length === 0) {
    return (
      <div className="py-16 text-center flex flex-col gap-2 items-center">
        ...
        >
          Link a test case to a plan (e.g. “Regressão v2.0”) from a card’s
          Sentinel tab.
        </p>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
      {plans.map((plan) => {
        ...
        const passed = plan.cases.filter(
          (c) => c.latest_status === "passed",
        ).length;
        return (
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  >
                    {passed}/{plan.cases.length} passed ·{" "}
                    {STATUS_META[rollup].label}
                  </div>
                </div>
              </div>
              <span
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      ...
                      color: STATUS_META[c.latest_status].color,
                    }}
                  >
