...
import { demoReorderSections, updateDemoBoard } from "@/lib/demoStorage";
    ...
    <div className="aero-menu rounded-2xl p-6 w-[90vw] max-w-md flex flex-col gap-5 relative max-h-[85vh] overflow-y-auto">
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          value={ticketPrefix}
          onChange={(e) =>
            setTicketPrefix(e.target.value.replace(/\s+/g, "").toUpperCase())
          }
          maxLength={10}
        ...
        <span
          ...
          style={{ color: "var(--cf-text-muted)" }}
        >
          Leave blank to show plain numbers (#42).
        </span>
      </div>
              ...
              <div className="flex flex-col gap-2">
                {sections.map((s, i) => (
                  <SortableRow
                    ...
                    color={
                      SECTION_COLORS[
                        initialSections.findIndex((orig) => orig.id === s.id) %
      ...
      >
        <button
          type="button"
          onClick={onDelete}
          className="aero-btn aero-btn--magenta uppercase tracking-widest font-bold px-4 py-1.5 text-[10px]"
        >
          Delete
        </button>
