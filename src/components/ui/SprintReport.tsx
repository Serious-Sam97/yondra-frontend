...
function localReport(
  ...
  const tickets: SprintReportTicket[] =
    sprint.report_snapshot && sprint.report_snapshot.length
      ? sprint.report_snapshot.map((t) => ({
          id: t.id,
          name: t.name,
          points: t.points ?? 0,
      ...
      : cards.map((c) => ({
          id: Number(c.id),
          name: c.name,
          points: toNumber(c.story_points),
  ...
  const committed =
    sprint.committed_points ?? tickets.reduce((s, t) => s + (t.points ?? 0), 0);
  const completedPts = completed.reduce((s, t) => s + (t.points ?? 0), 0);

  // Per-day burndown from done_at over the sprint window.
  const start = sprint.started_at
    ? new Date(sprint.started_at)
    : sprint.start_date
      ? new Date(sprint.start_date + "T00:00:00")
      : new Date();
  const end = sprint.completed_at
    ? new Date(sprint.completed_at)
    : sprint.end_date
      ? new Date(sprint.end_date + "T00:00:00")
      : new Date();
  ...
  const burndown = Array.from({ length: days }, (_, i) => {
    ...
    const burned = completed
      .filter((t) => t.done_at && new Date(t.done_at) <= day)
      .reduce((s, t) => s + (t.points ?? 0), 0);
    return {
      date: day.toISOString().slice(0, 10),
      remaining: Math.max(0, committed - burned),
      ideal:
        Math.round(committed * (1 - (days > 1 ? i / (days - 1) : 1)) * 10) / 10,
    };
  });
  return {
    sprint,
    committed_points: committed,
    completed_points: completedPts,
    completed,
...
function median(nums: number[]): number {
  ...
  return s.length % 2
    ? s[mid]
    : Math.round(((s[mid - 1] + s[mid]) / 2) * 10) / 10;
}
...
function byAssignee(tickets: SprintReportTicket[]) {
  const groups = new Map<
    string,
    { name: string; id: number | null; tickets: SprintReportTicket[] }
  >();
...
function Burndown({
  ...
  data: SprintReportData["burndown"];
  big?: boolean;
}) {
  const W = 320,
    H = big ? 200 : 150,
  ...
  return (
    ...
    >
      ...
      <polyline
        ...
        fill="none"
        stroke="var(--cf-phosphor)"
        strokeWidth="2.5"
        style={{ filter: "drop-shadow(0 0 4px var(--cf-phosphor))" }}
      />
      {data.map((d, i) => (
        <circle
          key={i}
...
function Velocity({
  ...
  const max = Math.max(
    1,
    ...done.map((s) =>
      Math.max(s.committed_points ?? 0, s.completed_points ?? 0),
    ),
  );
    ...
    >
      {done.map((s) => (
        ...
        >
          ...
          >
            <div
              title={`Committed ${s.committed_points ?? 0}`}
              style={{
                width: bw,
                height: px(s.committed_points ?? 0),
                background: "var(--cf-text-muted)",
              }}
            />
            <div
              title={`Completed ${s.completed_points ?? 0}`}
              style={{
                width: bw,
                height: px(s.completed_points ?? 0),
          ...
          <span
            className="cf-mono truncate w-full text-center"
            style={{
...
function MedianBars({
  ...
  people: ReturnType<typeof byAssignee>;
  overall: number;
  big?: boolean;
}) {
  const rows = [
    { name: "All tickets", id: -1 as number | null, median: overall },
    ...people,
  ];
    ...
    <div className="flex flex-col gap-1.5">
      {rows.map((r) => {
        ...
        const color = isAll
          ? "var(--cf-phosphor)"
          : r.id !== null
            ? AVATAR_COLORS[r.id % AVATAR_COLORS.length]
            : "var(--cf-text-muted)";
        return (
          <div key={String(r.id)} className="flex items-center gap-2">
            <span
              className="cf-mono truncate flex-shrink-0 text-right"
              style={{
                width: big ? 128 : 96,
                fontSize: big ? "12px" : "9px",
                color: isAll ? "var(--cf-text)" : "var(--cf-text-muted)",
            ...
            <div
              className={`flex-1 ${big ? "h-5" : "h-3.5"} rounded-sm overflow-hidden`}
            ...
            <span
              className="cf-mono tabular-nums flex-shrink-0 text-right"
              style={{
                width: big ? 34 : 26,
...
export function SprintReport({
  ...
  const [report, setReport] = useState<SprintReportData>(() =>
    localReport(sprint, cards),
  );

  useEffect(() => {
    if (isDemo || boardId === 0) return;
    getSprintReport(boardId, sprint.id)
      .then(setReport)
      .catch(() => {
        /* keep local fallback */
      });
  ...
  const pct = report.committed_points
    ? Math.round((report.completed_points / report.committed_points) * 100)
    : 0;
  ...
  const overallMedian = median(
    allTickets.map((t) => t.points ?? 0).filter((p) => p > 0),
  );
  ...
  const Tile = ({
    ...
    <div
      className={`glass-panel rounded-xl flex flex-col items-center gap-1.5 flex-1 ${isPage ? "px-4 py-5 min-w-[128px]" : "px-3 py-3.5 min-w-[88px]"}`}
      style={{ borderTop: `2px solid ${color}` }}
    >
      <p
        className="font-bold tabular-nums leading-none"
        style={{ fontSize: fs(30, 46), color }}
      >
        {value}
      </p>
      <p
        className="cf-mono uppercase tracking-widest text-center leading-tight"
        style={{ fontSize: fs(8, 11), color: "var(--cf-text-muted)" }}
      >
        {label}
      </p>
    </div>
  ...
  const medianPanel = (
    <div className={`${panelCls} gap-3`}>
      <p
        ...
        style={{ fontSize: fs(9, 12), color: "var(--cf-text-muted)" }}
      >
        Median story points
      </p>
  ...
  const assigneePanel = (
    <div className={`${panelCls} gap-2`}>
      ...
      >
        ...
        <span
          className="cf-mono uppercase tracking-widest text-right"
          style={{ fontSize: fs(8, 11), color: "var(--cf-text-muted)" }}
        >
          Tickets
        </span>
      ...
      {people.length === 0 ? (
        <p
          className="cf-mono"
          style={{ fontSize: fs(10, 13), color: "var(--cf-text-muted)" }}
        >
          —
        </p>
      ) : (
        people.map((p) => (
          <div
            key={p.id ?? "unassigned"}
            className={`grid items-center gap-2 ${isPage ? "py-0.5" : ""}`}
          ...
          >
            <span className="flex items-center gap-2 min-w-0">
              {p.id !== null ? (
                <span
                  ...
                  style={{
                    ...
                    height: avatar,
                    fontSize: fs(8, 10),
                    backgroundColor: AVATAR_COLORS[p.id % AVATAR_COLORS.length],
                    color: "var(--cf-ink)",
              ...
              <span
                ...
                style={{
                  fontSize: fs(11, 14),
                  color:
                    p.id !== null ? "var(--cf-text)" : "var(--cf-text-muted)",
                }}
              >
                {p.name}
              </span>
            ...
            <span
              className="cf-mono tabular-nums text-right"
              style={{ fontSize: fs(10, 13), color: "var(--cf-text)" }}
            >
              <span style={{ color: "var(--cf-phosphor)" }}>{p.done}</span>/
              {p.total}
            </span>
            <span
              className="cf-mono tabular-nums text-right"
              style={{ fontSize: fs(10, 13), color: "var(--cf-text)" }}
            >
              <span style={{ color: "var(--cf-phosphor)" }}>{p.donePts}</span>/
              {p.totalPts}
            </span>
            <span
              className="cf-mono tabular-nums text-right font-bold"
              style={{ fontSize: fs(10, 13), color: "var(--cf-cyan)" }}
            >
              {p.median}
            </span>
          </div>
  ...
  return (
    ...
    >
      {isPage && (
        <button
          onClick={onClose}
          className="cf-label flex items-center gap-1.5 cursor-pointer transition-colors duration-150 self-start"
      ...
      >
        <Icon
          icon={faChartLine}
          style={{ color: "var(--cf-phosphor)", fontSize: isPage ? 22 : 15 }}
        />
        <div className="flex flex-col">
          <p
            className="chrome-text font-bold"
            style={{ fontSize: isPage ? "26px" : "15px" }}
          >
            {sprint.name}
          </p>
          <p
            className="cf-mono uppercase tracking-[0.25em]"
            style={{ fontSize: fs(8, 10), color: "var(--cf-text-muted)" }}
          >
            Sprint report
          </p>
        </div>
        {!isPage && (
          <button
            onClick={onClose}
            aria-label="Close"
            className="ml-auto btn-physical cursor-pointer"
            style={{ color: "var(--cf-text-muted)" }}
          >
            <Icon icon={faXmark} />
          </button>
      ...
      >
        <Tile
          label="Committed pts"
          value={report.committed_points}
          color="var(--cf-text)"
        />
        <Tile
          label="Completed pts"
          value={report.completed_points}
        ...
        <Tile
          label="Not done"
          value={report.not_completed.length}
          color="var(--cf-amber)"
        />
      ...
      <div className="flex items-center gap-3">
        <div
          className={`flex-1 ${isPage ? "h-3" : "h-2"} rounded-full overflow-hidden`}
          style={{
            background: "#0d1410",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
        <div className={`${panelCls} gap-2`}>
          ...
          <div className="flex gap-3">
            <span
              className="cf-mono inline-flex items-center gap-1"
              style={{ fontSize: fs(8, 11), color: "var(--cf-text-muted)" }}
            >
              <span
                style={{
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
        <TicketList
          title={`Completed (${report.completed.length})`}
          tickets={report.completed}
          color="var(--cf-phosphor)"
          big={isPage}
        />
        <TicketList
          title={`Not completed (${report.not_completed.length})`}
          tickets={report.not_completed}
          color="var(--cf-amber)"
          big={isPage}
        />
      </div>
...
function TicketList({
  ...
  return (
    <div
      className={`glass-panel rounded-xl ${big ? "p-5 gap-2.5" : "p-4 gap-2"} flex flex-col`}
    >
      <p
        className="cf-mono uppercase tracking-widest font-bold"
        style={{ fontSize: big ? "12px" : "9px", color }}
      >
        {title}
      </p>
      {tickets.length === 0 ? (
        ...
        tickets.map((t) => (
          <div
            key={t.id}
            className={`flex items-center gap-2 ${big ? "py-1 border-b last:border-b-0" : ""}`}
            style={big ? { borderColor: "var(--cf-edge)" } : undefined}
          >
            <span
