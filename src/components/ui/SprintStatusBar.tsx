...
export function SprintStatusBar({
  sprint,
  sections,
  sprintCards,
  canManage,
  ...
  const columnPoints = sections.map((sec) => ({
    ...
    points: sprintCards
      ...
      .reduce((sum, c) => sum + toNumber(c.story_points), 0),
  }));
  ...
  return (
    <div className="glass-panel flex items-center gap-3 flex-wrap px-4 py-2.5 rounded-2xl mb-4">
      <span
        className="cf-mono uppercase tracking-widest font-bold flex items-center gap-2 flex-shrink-0"
        style={{ fontSize: "13px", color: "var(--cf-text)" }}
      >
        <Icon
          icon={faBolt}
          style={{ color: "var(--cf-phosphor)", fontSize: 13 }}
        />
        {sprint.name}
        <span
          className="cf-led"
      ...
      {sprint.goal && (
        <span
          className="cf-mono truncate hidden lg:block"
          style={{
            fontSize: "12px",
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {columnPoints.map((col) => (
          <span
            key={col.name}
            className="cf-mono px-2.5 py-1 rounded-sm tabular-nums"
            style={{
              fontSize: "11px",
          ...
          >
            {col.name}{" "}
            <span className="font-bold" style={{ color: "var(--cf-cyan)" }}>
              {col.points}
            </span>
          </span>
        ))}
      ...
      <div className="flex items-center gap-2 ml-auto flex-shrink-0">
        ...
        {canManage && (
          ...
          >
            <Icon icon={faFlagCheckered} style={{ fontSize: "11px" }} />{" "}
            Complete sprint
          </button>
        )}
      </div>
