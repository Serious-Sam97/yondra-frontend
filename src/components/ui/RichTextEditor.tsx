...
export interface MentionUser { id: number; name: string }
...
interface MentionListHandle { onKeyDown: (e: KeyboardEvent) => boolean }

const MentionList = forwardRef<MentionListHandle, { items: MentionUser[]; command: (u: { id: string; label: string }) => void }>(
    function MentionList({ items, command }, ref) {
        ...
        useImperativeHandle(ref, () => ({
            onKeyDown: (e: KeyboardEvent) => {
                ...
                if (e.key === 'Enter')     { pick(selected); return true }
                return false
            },
        }))
        ...
        return (
            <div className="aero-menu flex flex-col rounded-lg overflow-hidden" style={{ minWidth: 160, background: '#1c1a16', border: '1px solid var(--cf-edge)' }}>
                {items.map((u, i) => (
                    <button
                        key={u.id}
                        type="button"
                        ...
                        style={{ fontSize: 12, color: i === selected ? 'var(--cf-phosphor)' : 'var(--cf-text)', background: i === selected ? 'rgba(255,255,255,0.06)' : 'transparent' }}
                    >
                        @{u.name.split(' ')[0]} <span style={{ fontSize: 10, color: 'var(--cf-text-muted)' }}>{u.name}</span>
                    </button>
                ))}
            </div>
        )
...
function mentionSuggestion(users: MentionUser[]) {
    return {
        items: ({ query }: { query: string }) =>
            users.filter(u => mentionHandle(u).toLowerCase().startsWith(query.toLowerCase())).slice(0, 6),
        render: () => {
            ...
            return {
                onStart: (props: { clientRect?: (() => DOMRect | null) | null }) => {
                    component = new ReactRenderer(MentionList, { props, editor: (props as unknown as { editor: Editor }).editor })
                ...
                onExit: () => { el?.remove(); el = null; component?.destroy(); component = null },
            }
        },
    }
}
...
function ToolbarButton({ active, onClick, title, children }: { active?: boolean; onClick: () => void; title: string; children: React.ReactNode }) {
    return (
        <button
            ...
            style={{
                minWidth: 26, height: 26, padding: '0 6px', fontSize: 12, fontWeight: 700,
...
const Sep = () => <span style={{ width: 1, height: 18, background: 'var(--cf-edge)', margin: '0 2px' }} />

function Toolbar({ editor, onPickImage, uploading, compact }: { editor: Editor; onPickImage: () => void; uploading: boolean; compact?: boolean }) {
    ...
    return (
        <div className="flex flex-wrap items-center gap-1 pb-2 mb-2" style={{ borderBottom: '1px solid var(--cf-edge)' }}>
            ...
            {!compact && <ToolbarButton title="Table" onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()}>▦</ToolbarButton>}
            <ToolbarButton title={uploading ? 'Uploading…' : 'Insert image'} onClick={onPickImage}>{uploading ? '…' : '🖼'}</ToolbarButton>
        </div>
    )
}

// ── Editor ──────────────────────────────────────────────────────────────────
export default function RichTextEditor({ value, onChange, editable = true, placeholder, onUploadImage, mentionUsers, compact }: RichTextEditorProps) {
    ...
    const extensions = [
        ...
        ...(mentionUsers ? [Mention.configure({
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: { class: `rich-content ProseMirror-focused${compact ? ' rt-compact' : ''}` },
            handlePaste: (_view, event) => handleFiles(Array.from(event.clipboardData?.files ?? []), event),
            handleDrop:  (_view, event) => handleFiles(Array.from((event as DragEvent).dataTransfer?.files ?? []), event),
        },
    })
    ...
    const uploadAndInsert = async (file: File) => {
        ...
        } catch { /* surfaced by caller's error UI */ }
    ...
    return (
        <div className="flex flex-col">
            {editable && <Toolbar editor={editor} uploading={uploading} compact={compact} onPickImage={() => fileInputRef.current?.click()} />}
            <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
