...
const groupIdFor = (sprintId: number | null) =>
  sprintId === null ? "backlog" : `sprint-${sprintId}`;
const sprintIdFromGroup = (groupId: string): number | null =>
  groupId === "backlog" ? null : Number(groupId.replace("sprint-", ""));
...
function TicketRow({
  ...
  return (
    ...
    >
      ...
      <span
        className="cf-led rounded-full flex-shrink-0"
        style={{
          width: 9,
          height: 9,
          ...
          boxShadow: card.priority
            ? `0 0 6px ${PRIORITY_COLOR[card.priority]}`
            : "none",
        }}
      ...
      >
        {card.ticket_key && (
          <span
            className="cf-mono font-bold flex-shrink-0"
      ...
      {(card.tags ?? []).length > 0 && (
        <div className="hidden md:flex items-center gap-1 flex-shrink-0">
          {(card.tags ?? []).slice(0, 2).map((tag) => (
            <span
              key={tag.id}
              className="aero-pill cf-mono uppercase tracking-wide px-1.5 py-0.5 font-bold whitespace-nowrap"
              style={{
                fontSize: "10px",
                color: tag.color,
                borderColor: tag.color + "88",
                boxShadow: `0 0 6px ${tag.color}44`,
              }}
            >
              {tag.name}
            </span>
      ...
      {card.priority && (
        <span
          className="cf-mono uppercase tracking-widest font-bold flex-shrink-0 hidden lg:inline"
          style={{ fontSize: "10px", color: PRIORITY_COLOR[card.priority] }}
        >
          {card.priority}
        </span>
      )}
      ...
      {card.story_points != null && (
        <span
          className="cf-mono tabular-nums flex-shrink-0 font-bold"
          style={{ fontSize: "12px", color: "var(--cf-cyan)" }}
        >
          {toNumber(card.story_points)} pts
        </span>
      )}

      {/* Assignee */}
      {card.assigned_user ? (
        <Avatar user={card.assigned_user} />
      ) : (
        <div
          className="rounded-full flex-shrink-0"
          style={{ width: 24, height: 24, border: "1px dashed var(--cf-edge)" }}
          title="Unassigned"
        />
      )}
      {canManage && (
  ...
  return (
    <div
      ...
      style={
        isOver
          ? {
              boxShadow: "0 0 0 2px var(--cf-phosphor)",
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        >
          ...
          <span style={{ fontSize: "11px", color: "var(--cf-text-muted)" }}>
            {cards.length} · {points} pts
          </span>
          {subtitle && (
            <span
              className="cf-mono truncate hidden md:inline"
          ...
          <div className="overflow-hidden min-h-0">
            {cards.length === 0 && (
              <p
                ...
                style={{
                  fontSize: "10px",
                  color: isOver ? "var(--cf-phosphor)" : "var(--cf-text-muted)",
                  borderTop: "1px solid var(--cf-edge)",
                }}
              >
            ...
            {canManage && quickCreate && (
              ...
              >
                ...
                <button
                  onClick={submit}
                  disabled={!draft.trim()}
                  className="aero-btn aero-btn--cyan text-[11px] uppercase tracking-widest font-bold px-3 py-1.5 cursor-pointer disabled:opacity-40"
                >
                  Add
                </button>
              </div>
...
function daysUntil(end: string) {
  ...
  t.setHours(0, 0, 0, 0);
  return Math.round(
    (new Date(end + "T00:00:00").getTime() - t.getTime()) / 86400000,
  );
}
...
function SprintDates({
  ...
  const [end, setEnd] = useState(
    sprint.end_date ?? addDays(sprint.start_date ?? todayStr(), 14),
  );
  const dur =
    sprint.start_date && sprint.end_date
      ? diffDays(sprint.start_date, sprint.end_date)
      : null;
  const left =
    sprint.status === "active" && sprint.end_date
      ? daysUntil(sprint.end_date)
      : null;

  if (editing && canManage && onUpdate) {
    return (
      <div
        className="flex items-center gap-2 flex-wrap px-3 pb-3"
        style={{ marginTop: -2 }}
      >
        <input
          type="date"
          ...
          onChange={(e) => {
            ...
            setStart(v);
            if (dur != null && diffDays(v, end) < 0) setEnd(addDays(v, dur));
          }}
          className="glass-input cf-lcd"
          style={{ fontSize: "13px", padding: "4px 6px" }}
        />
        <span style={{ color: "var(--cf-text-muted)", fontSize: "12px" }}>
          →
        </span>
        <input
          type="date"
        ...
        <button
          onClick={() => {
            onUpdate({ start_date: start, end_date: end });
            setEditing(false);
          }}
          className="aero-btn aero-btn--cyan text-[11px] uppercase tracking-widest font-bold px-3 py-1.5 cursor-pointer"
        >
          Save
        </button>
        <button
          onClick={() => {
            setStart(sprint.start_date ?? todayStr());
            setEnd(sprint.end_date ?? todayStr());
            setEditing(false);
          }}
          className="aero-btn aero-btn--ghost text-[11px] uppercase tracking-widest px-3 py-1.5 cursor-pointer"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <div
      className="flex items-center gap-2.5 px-3 pb-3 flex-wrap"
      style={{ marginTop: -2 }}
    >
      <span
        className="cf-mono inline-flex items-center gap-1.5"
        style={{ fontSize: "12px", color: "var(--cf-text-muted)" }}
      >
        <Icon icon={faCalendarDays} style={{ fontSize: 12 }} />{" "}
        {fmtDate(sprint.start_date)} – {fmtDate(sprint.end_date)}
        {dur != null && <span>· {dur}d</span>}
      </span>
      {left != null && (
        <span
          className="cf-mono font-bold"
          style={{
            fontSize: "12px",
            color:
              left < 0
      ...
      {canManage && onUpdate && (
        <button
          ...
          title="Edit dates"
          className="btn-physical cursor-pointer"
          style={{ color: "var(--cf-text-muted)", fontSize: "12px" }}
        >
          <Icon icon={faPen} />
        </button>
      )}
...
function CreateSprintForm({
  ...
  const reset = () => {
    ...
    setWeeks(2);
    setEnd(addDays(suggestedStart, 14));
  };
  const changeStart = (s: string) => {
    setStart(s);
    if (weeks) setEnd(addDays(s, weeks * 7));
  };
  const changeWeeks = (w: number | null) => {
    setWeeks(w);
    if (w) setEnd(addDays(start, w * 7));
  };
  const submit = () => {
    const n = name.trim();
    if (!n) return;
    onCreate({ name: n, start_date: start, end_date: end });
    reset();
    setOpen(false);
  };
  ...
  return (
    <div className="glass-panel rounded-xl p-4 mb-4 flex flex-col gap-3">
      <input
        ...
        style={{ maxWidth: 340, padding: "6px 10px" }}
      />
      <div className="flex gap-3 flex-wrap items-end">
        <div className="flex flex-col gap-1.5">
          <label className="cf-mono uppercase tracking-widest" style={label}>
            Start
          </label>
          <input
            type="date"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select
            value={weeks ?? ""}
            onChange={(e) =>
              changeWeeks(e.target.value === "" ? null : Number(e.target.value))
            }
            className="glass-input cf-lcd cursor-pointer"
            style={{ fontSize: "14px", padding: "6px 8px" }}
          >
            {DURATIONS.map((d) => (
              <option key={d.weeks} value={d.weeks} className="text-black">
                {d.label}
              </option>
            ))}
      ...
      <div className="flex gap-2">
        <button
          onClick={submit}
          disabled={!name.trim()}
          className="aero-btn aero-btn--cyan text-[11px] uppercase tracking-widest font-bold px-4 py-2 cursor-pointer disabled:opacity-40"
        >
          Create sprint
        </button>
        <button
          onClick={() => setOpen(false)}
          className="aero-btn aero-btn--ghost text-[11px] uppercase tracking-widest px-4 py-2 cursor-pointer"
        >
          Cancel
        </button>
      </div>
...
export function SprintBacklog({
  ...
  const handleDragStart = (e: DragStartEvent) =>
    setDragCard(e.active.data.current?.card ?? null);
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if ((card.sprint_id ?? null) !== target)
      onAssignSprint(Number(card.id), target);
  };
  ...
  const targets: { id: number | null; label: string }[] = [
    ...
    ...(activeSprint
      ? [{ id: activeSprint.id, label: `${activeSprint.name} (active)` }]
      : []),
  ...
  return (
    ...
    >
      <div
        className="flex flex-col pb-8 w-full mx-auto"
        style={{ maxWidth: 900 }}
      >
          ...
          <Group
            ...
            subheader={
              <SprintDates
                ...
                onUpdate={
                  canManage
                    ? (dates) => onUpdateSprintDates(activeSprint.id, dates)
                    : undefined
                }
              />
            }
            headerActions={
              <button
                onClick={() => onOpenReport(activeSprint)}
                title="Sprint report"
                className="btn-physical px-1.5 cursor-pointer"
                style={{ color: "var(--cf-text-muted)", fontSize: "11px" }}
              >
          ...
          <Group
            ...
            subheader={
              <SprintDates
                ...
                onUpdate={
                  canManage
                    ? (dates) => onUpdateSprintDates(s.id, dates)
                    : undefined
                }
              />
            }
            quickCreate={
              canManage ? (name) => onQuickCreate(name, s.id) : undefined
            }
            headerActions={
              canManage && (
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <button
                    ...
                    title="Delete sprint"
                    className="btn-physical px-1.5 cursor-pointer"
                    style={{ color: "var(--cf-text-muted)", fontSize: "10px" }}
                  >
                    <Icon icon={faTrash} />
                  </button>
                </div>
        ...
        <Group
          ...
          canManage={canManage}
          quickCreate={
            canManage ? (name) => onQuickCreate(name, null) : undefined
          }
          headerActions={
            <Icon
              icon={faLayerGroup}
        ...
        {completedSprints.length > 0 && (
          <div className="mt-2">
            <p
              className="cf-mono uppercase tracking-widest font-bold mb-2"
              style={{ fontSize: "11px", color: "var(--cf-text-muted)" }}
            >
              Completed
            </p>
              ...
              {completedSprints.map((s) => (
                ...
                >
                  <Icon
                    ...
                    style={{ color: "var(--cf-phosphor)", fontSize: "12px" }}
                  />
                  <span
                    className="cf-mono uppercase tracking-widest font-bold flex-1 truncate"
                  ...
                  <span
                    ...
                    style={{ fontSize: "11px", color: "var(--cf-text-muted)" }}
                  >
                    {s.completed_points ?? 0}/{s.committed_points ?? 0} pts
                  </span>
                  <span
                    className="aero-pill cf-mono inline-flex items-center gap-1 px-2 py-1 font-bold"
                    style={{ fontSize: "11px", color: "var(--cf-cyan)" }}
                  >
                    <Icon icon={faChartLine} /> Report
                  </span>
                </button>
              ))}
            </div>
      ...
      <DragOverlay dropAnimation={null}>
        {dragCard ? (
          <div
            className="glass-panel flex items-center gap-2 px-3 py-2 rounded-lg"
            style={{
          ...
          >
            <span
              ...
              style={{
                background: dragCard.priority
                  ? PRIORITY_COLOR[dragCard.priority]
                  : "var(--cf-edge)",
              }}
            />
            <span
            ...
            {dragCard.story_points != null && (
              <span
                className="cf-mono"
                style={{ fontSize: "9px", color: "var(--cf-cyan)" }}
              >
                {toNumber(dragCard.story_points)} pts
              </span>
            )}
          </div>
        ) : null}
