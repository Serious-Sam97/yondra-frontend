...
function calendarDays(year: number, month: number) {
  ...
  for (let i = 0; i < first.getDay(); i++) {
    days.push({
      date: new Date(year, month, i - first.getDay() + 1),
      current: false,
    });
  }
...
export function CalendarView({
  ...
  const byDate: Record<
    string,
    { card: CardInterface; kind: "due" | "done" }[]
  > = {};
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <h2 className="chrome-text cf-lcd font-bold text-lg tracking-wide">
            {MONTHS[month]}{" "}
            <span style={{ color: "var(--cf-text-muted)" }}>{year}</span>
          </h2>
          <button
            onClick={goToday}
            className="aero-pill cf-label text-[10px] uppercase tracking-widest cursor-pointer transition-colors px-2 py-1"
      ...
      {totalWithDates === 0 ? (
        <p
          className="cf-label text-center text-xs uppercase tracking-widest py-4"
          style={{ color: "var(--cf-text-muted)" }}
        >
        ...
        <div
          className="flex items-center justify-center gap-4 cf-label text-[10px] uppercase tracking-widest"
          style={{ color: "var(--cf-text-muted)" }}
        >
          <span className="flex items-center gap-1.5">
            <span
              className="w-2.5 h-2.5 rounded-sm"
              style={{ border: "1px solid var(--cf-text-muted)" }}
            />{" "}
            Due
          </span>
          <span className="flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-7">
        {DAYS_SHORT.map((d) => (
          <div
            key={d}
            className="cf-label text-center text-[10px] uppercase tracking-widest pb-2 font-bold"
            style={{ color: "var(--cf-text-muted)" }}
          >
            {d}
          </div>
            ...
            <div
              ...
              style={{
                ...
                backgroundColor: current
                  ...
                  : "var(--cf-screen)",
                boxShadow: isToday
                  ? "inset 0 0 0 1.5px var(--cf-phosphor), inset 0 0 18px rgba(154,166,126,0.22)"
                  : "none",
              }}
            >
              {/* Day number */}
              <span
                className={`cf-mono text-[11px] font-bold w-5 h-5 flex items-center justify-center rounded-full mb-0.5 flex-shrink-0 self-start`}
                style={
                  isToday
                    ? {
                ...
                return (
                  <button
                    ...
                    style={
                      isDone
                        ? {
                            borderColor: "rgba(154,166,126,0.6)",
                            boxShadow: "0 0 8px rgba(154,166,126,0.45)",
                            opacity: 0.9,
                          }
                        : {
                  ...
                  >
                    {isDone ? (
                      <span
                        className="mr-0.5"
                        style={{ fontSize: "8px", color: "var(--cf-phosphor)" }}
                      >
                        <Icon icon={faCheck} />
                      </span>
                    ) : card.priority === "high" ? (
                      <span
                        className="cf-led mr-0.5"
                        style={{ fontSize: "8px", color: "var(--cf-red)" }}
                      >
                        ●
                      </span>
                    ) : null}
              ...
              {dayCards.length > 3 && (
                <span
                  className="cf-mono text-[9px] px-1"
                  style={{ color: "var(--cf-text-muted)" }}
                >
