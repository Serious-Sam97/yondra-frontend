...
const GRID = 'grid grid-cols-[28px_1fr_auto_auto] md:grid-cols-[28px_1fr_130px_110px_96px_36px]'

export function BacklogView({ cards, users, onCardClick, onAddToBoard, onQuickCreate, onOpenEditor, canPromote, isReadOnly = false }: BacklogViewProps) {
    ...
    return (
        <div className="flex flex-col pb-8">
            ...
            {/* Header */}
            <div className={`${GRID} aero-column rounded-b-none overflow-hidden`} style={{ borderBottom: '1px solid var(--cf-edge)' }}>
            ...
            <div className="aero-column rounded-t-none overflow-hidden">
                {sorted.length === 0 && (
                    <p className="cf-label text-center text-xs uppercase tracking-widest py-12" style={{ color: 'var(--cf-text-muted)' }}>
                            ...
                            <div className="px-2 py-3 hidden md:flex items-center md:border-l md:border-white/10">
                                {!isReadOnly && canPromote && (
                                    <span
                                        role="button"
                                        tabIndex={0}
                                        onClick={(e) => { e.stopPropagation(); onAddToBoard(card) }}
                                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); onAddToBoard(card) } }}
