...
type SortKey = "name" | "priority" | "due_date";
...
const GRID =
  "grid grid-cols-[28px_1fr_auto_auto] md:grid-cols-[28px_1fr_130px_110px_96px_36px]";
...
export function BacklogView({
  ...
  const SortIcon = ({ k }: { k: SortKey }) => (
    <span className="ml-1 text-[8px] opacity-60">
      {sortKey === k ? (
        <Icon icon={sortDir === "asc" ? faSortUp : faSortDown} />
      ) : (
        <Icon icon={faSort} />
      )}
    </span>
  ...
  return (
    <div className="flex flex-col pb-8">
      ...
      {!isReadOnly && (
        <div className="flex items-center gap-2 mb-3">
          <input
            ...
            placeholder="Add a backlog ticket…"
            className="glass-input flex-1 text-xs"
          />
          <button
            onClick={submitDraft}
            disabled={!draft.trim()}
            className="aero-btn aero-btn--cyan text-[10px] uppercase tracking-widest font-bold px-3 py-2 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed inline-flex items-center gap-1.5"
          >
            <Icon icon={faPlus} /> Add
          </button>
          <button
            onClick={onOpenEditor}
            className="aero-btn aero-btn--ghost text-[10px] uppercase tracking-widest font-bold px-3 py-2 cursor-pointer whitespace-nowrap"
            title="Create with the full editor"
          >
            Full editor
          </button>
        </div>
      ...
      {/* Header */}
      <div
        className={`${GRID} aero-column rounded-b-none overflow-hidden`}
        style={{ borderBottom: "1px solid var(--cf-edge)" }}
      >
        <div
          className="cf-label px-2 py-2.5 text-[10px] uppercase tracking-widest font-bold flex items-center justify-center"
          style={{ color: "var(--cf-text-muted)" }}
          title="Priority"
        >
          P
        ...
        <ColHeader
          k="name"
          label="Name"
          className="md:border-l md:border-white/15"
        />
        <div
          className="cf-label px-3 py-2.5 text-[10px] uppercase tracking-widest font-bold md:border-l md:border-white/15 hidden md:flex items-center"
          style={{ color: "var(--cf-text-muted)" }}
        >
          Tags
        </div>
      ...
      <div className="aero-column rounded-t-none overflow-hidden">
        {sorted.length === 0 && (
          <p
            className="cf-label text-center text-xs uppercase tracking-widest py-12"
            style={{ color: "var(--cf-text-muted)" }}
          >
            Backlog is empty — park a ticket here
          </p>
        )}
        {sorted.map((card) => {
          ...
          return (
            <button
              ...
              className={`${GRID} w-full text-left hover:bg-white/10 transition-colors cursor-pointer group last:border-0`}
              style={{ borderBottom: "1px solid var(--cf-edge)" }}
            >
              {/* Priority */}
                ...
                <div
                  ...
                  style={{
                    ...
                    boxShadow: card.priority
                      ? `0 0 8px ${PRIORITY_COLOR[card.priority]}`
                      : "none",
                  }}
                />
              </div>
              ...
              <div className="px-3 py-3 flex items-center gap-2 min-w-0 md:border-l md:border-white/10">
                {card.ticket_key && (
                  <span
                    className="cf-mono text-xs font-bold flex-shrink-0"
                    style={{ color: "var(--cf-text-muted)" }}
                  >
                    {card.ticket_key}
                  </span>
                ...
                <span
                  className="cf-mono text-sm truncate transition-colors leading-snug"
                ...
                {(card.tags ?? []).slice(0, 2).map((tag) => (
                  <span
                    ...
                    style={{
                      ...
                      boxShadow: `0 0 7px ${tag.color}55`,
                    }}
                  >
                    {tag.name}
                  </span>
              ...
              <div className="px-3 py-3 flex items-center md:border-l md:border-white/10">
                {due ? (
                  ...
                  <span style={{ color: "var(--cf-edge)" }} className="text-sm">
                    —
                  </span>
                )}
              </div>
              ...
              {/* Add to Board action */}
              <div className="px-2 py-3 hidden md:flex items-center md:border-l md:border-white/10">
                {!isReadOnly && canPromote && (
                  <span
                    ...
                    className="aero-btn aero-btn--cyan cf-mono text-[9px] uppercase tracking-widest font-bold px-2 py-1 cursor-pointer inline-flex items-center gap-1 whitespace-nowrap"
                    title="Move this ticket onto the board (To Do)"
                  >
                    <Icon icon={faArrowRightToBracket} /> Board
                  </span>
                )}
              </div>
      ...
      {sorted.length > 0 && (
        <p
          className="cf-label text-[10px] uppercase tracking-widest text-right pt-2"
          style={{ color: "var(--cf-text-muted)" }}
        >
