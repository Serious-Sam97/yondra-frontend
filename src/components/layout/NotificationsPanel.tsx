...
export default function NotificationsPanel({
    ...
    return (
        <>
            ...
            <div className={`modal-content aero-menu absolute ${align === 'left' ? 'left-2' : 'right-2'} z-50 w-80 overflow-hidden`} style={{ top: anchorTop, maxHeight: '420px', ...extraStyle }}>
                <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--cf-edge)' }}>
                    ...
                    <div className="flex items-center gap-3">
                        {unreadCount > 0 && (
                            <button onClick={onMarkAll} className="btn-physical text-xs cursor-pointer cf-mono" style={{ color: 'var(--cf-phosphor)' }}>Mark all read</button>
                        )}
                        <button onClick={onClose} className="btn-physical text-xs cursor-pointer" style={{ color: 'var(--cf-text-muted)' }}>✕</button>
                    </div>
                </div>
