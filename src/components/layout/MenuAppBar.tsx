...
export default function MenuAppBar() {
    ...
    // Stand down on the dashboard — its sidebar owns the notification subscription there.
    const onDashboard = !!pathname?.startsWith('/dashboard');
    const { notifications, unreadCount, markOneRead, markAllRead } = useNotifications(user?.id, !onDashboard);
    ...
    // so the global header steps aside there (its bell + user menu live in the
    // sidebar, driven by the same useNotifications hook).
