...
import ProjectEdit, { type ProjectEditData } from "@/components/ui/ProjectEdit";
import {
  createDemoBoard,
  type DemoBoard,
  deleteDemoBoard,
  ...
  const handleSubmit = (project: ProjectEditData) => {
    if (project.id !== null) {
      ...
      setBoards((prev) =>
        prev.map((b) =>
          b.id === project.id
            ? { ...b, name: project.name, description: project.description }
            : b,
        ),
      );
    } else {
    ...
    <div className="min-h-screen px-4 py-6 md:px-8 md:py-10 max-w-6xl mx-auto">
      ...
      <div className="mb-10 glass-panel relative px-5 py-5">
        ...
        <div className="flex gap-2 mb-3">
          ...
          <span className="cf-label" style={{ color: "var(--cf-text-muted)" }}>
            PWR · LINK · DISK
          </span>
        </div>
        <p
          className="cf-label text-xs uppercase tracking-[0.3em] mb-1"
          style={{ color: "var(--cf-amber)" }}
        >
          Demo mode
        </p>
        <p className="chrome-text cf-mono text-3xl md:text-5xl font-bold tracking-tight">
          YOUR BOARDS
        </p>
        <p className="text-sm mt-2" style={{ color: "var(--cf-text-muted)" }}>
          Everything is saved in your browser.{" "}
          <span
            className="cf-mono cursor-pointer hover:underline"
      ...
      {boards.length === 0 ? (
        <div className="aero-column flex flex-col items-center justify-center py-24 text-center">
          <p
            className="cf-mono text-4xl mb-3"
            style={{ color: "var(--cf-phosphor)" }}
          >
            ▦
          </p>
          <p
            className="cf-mono text-sm uppercase tracking-widest"
            style={{ color: "var(--cf-text)" }}
          >
            NO BOARDS YET
          </p>
          <p
            className="cf-label text-xs mt-1"
            style={{ color: "var(--cf-text-muted)" }}
          >
            Click{" "}
            <span style={{ color: "var(--cf-phosphor)" }}>+ NEW BOARD</span> to
            get started
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <p
                    className="cf-mono font-bold text-lg leading-tight mb-2"
      ...
      {modalIsVisible && (
        <Modal>
          <div className="aero-menu p-6 w-[90%] max-w-lg">
            <p
              className="cf-label text-xs uppercase tracking-widest mb-4"
              style={{ color: "var(--cf-amber)" }}
            >
            ...
            <ProjectEdit
              ...
              submit={handleSubmit}
              onDelete={
                boardSelected ? () => handleDelete(boardSelected.id) : undefined
              }
