...
export interface Board {
  ...
  name: string;
  cards_count: number;
  shared_with: { id: number; permission?: string }[];
}
...
export async function mockApp(
  page: Page,
  opts: { currentUserId: number; project: ProjectFixture },
) {
  ...
  await page.route("**/api/**", (route) => {
    ...
    if (path.endsWith("/api/user"))
      return json(route, { id: me.id, name: me.name, email: me.email });
...
export function legacyBoardFixture() {
  ...
  const cards = [
    ...
    mk(168, 7, 41, "ToDo B"),
    mk(170, 7, 42, "ToDo C"),
    mk(113, 8, 19, "InProg A"),
    mk(292, 8, 43, "InProg B"),
  ];
 ...
 */
export async function mockBoard(
  page: Page,
  opts: {
    reorderStatus?: number;
    board?:
      | ReturnType<typeof boardFixture>
      | ReturnType<typeof legacyBoardFixture>;
  } = {},
) {
  ...
  await page.route("**/api/**", (route) => {
    ...
    if (path.endsWith("/api/boards/1/cards/reorder"))
      return json(route, { ok: true }, opts.reorderStatus ?? 200);
...
export async function dragCard(
  ...
  source: Locator,
  target: Locator,
  opts: { dropOffsetY?: number } = {},
) {
  const s = await source.boundingBox();
  const t = await target.boundingBox();
  if (!s || !t)
    throw new Error("dragCard: source or target has no bounding box");
